#include "../../../includes/textures/door_texture/door_five_texture.h"
...
*/
DoorFiveTexture::DoorFiveTexture(BigTexture & bigTexture, Area areaMap) 
:    Texture(bigTexture, areaMap, 
        std::move(
            std::unique_ptr<SpriteStrategy>(new SpriteStrategy(
                std::move(std::unique_ptr<DynamicSprite>(new DoorFiveClosedSprite()))
            ))
        )
    ) {}

/*Destruye la textura de la puerta con el numero 5.*/
DoorFiveTexture::~DoorFiveTexture() = default;
