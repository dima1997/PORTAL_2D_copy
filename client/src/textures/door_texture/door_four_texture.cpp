#include "../../../includes/textures/door_texture/door_four_texture.h"
...
*/
DoorFourTexture::DoorFourTexture(BigTexture & bigTexture, Area areaMap) 
:   Texture(bigTexture, areaMap, 
        std::move(
            std::unique_ptr<SpriteStrategy>(new SpriteStrategy(
                std::move(std::unique_ptr<DynamicSprite>(
                    new DoorFourClosedSprite())
                )
            ))
        )
    ) {}

/*Destruye la textura de la puerta con el numero 4.*/
DoorFourTexture::~DoorFourTexture() = default;
