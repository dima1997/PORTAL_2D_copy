#include "../../../includes/textures/door_texture/door_three_texture.h"
...
*/
DoorThreeTexture::DoorThreeTexture(BigTexture & bigTexture, Area areaMap) 
:   Texture(bigTexture, areaMap, 
        std::move(
            std::unique_ptr<SpriteStrategy>(new SpriteStrategy(
                std::move(std::unique_ptr<DynamicSprite>(
                    new DoorThreeClosedSprite()
                ))
            ))
        )
    ) {}

/*Destruye la textura de la puerta con el numero 3.*/
DoorThreeTexture::~DoorThreeTexture() = default;
