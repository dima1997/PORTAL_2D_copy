...
void Game::run(){
    ...
    BlockingQueue<GameObjectAction> changesAsk;
    ...
    //const Area & areaChell = window.getMainTextureArea(); //Lo necesitare oara enviar el angulo de disparo
    this->threads.push_back(std::move(std::unique_ptr<Thread>(new KeyReaderThread(this->player_id, changesAsk, this))));
    ...
    cv.wait(l, [this]{ return this->is_dead();});
}
